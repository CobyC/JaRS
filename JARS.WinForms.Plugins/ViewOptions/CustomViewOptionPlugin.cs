...
{
    [ExportPluginAsViewOption(typeof(IPluginAsViewOption), typeof(IViewOptionCustomEntity), "Custom", "CUSTOM")]
    [PartCreationPolicy(CreationPolicy.NonShared)]
    public class CustomViewOptionPlugin : ViewOptionPluginBase, IPluginAsViewOption
    {
        ...
        public BarCheckItem BarCheckItem
        {
            get
            {
                if (_BarCheckItem == null)
                {
                    ...
                    {
                        Caption = PluginText,
                        Glyph = Properties.Resources.ViewTools_16x16,
                        LargeGlyph = Properties.Resources.ViewTools_32x32,
                        Id = 605
                    };
        ...
        private void _BarItem_CheckedChanged(object sender, ItemClickEventArgs e)
        {
            ...
            if (item.Checked)
            {
                ...
                var lblResponse = Context.ServiceClient.Get(new FindApptLabels() { ViewType = "CUSTOM", InterfaceTypeName = typeof(IViewOptionCustomEntity).AssemblyQualifiedName });
                if (!lblResponse.IsErrorResponse())
                    SchedulerLabels = lblResponse.Labels.ConvertAll(l => l.ConvertTo<ApptLabel>());
                var stsResponse = Context.ServiceClient.Get(new FindApptStatuses() { ViewType = "CUSTOM", InterfaceTypeName = typeof(IViewOptionCustomEntity).AssemblyQualifiedName });
