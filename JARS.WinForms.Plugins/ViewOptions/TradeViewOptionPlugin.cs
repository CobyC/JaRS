...
{
    [ExportPluginAsViewOption(typeof(IPluginAsViewOption), typeof(IEntityWithLineOfWork), "Line Of Work", "LINEOFWORK")]
    [PartCreationPolicy(CreationPolicy.NonShared)]
    public class LineOfWorkViewOptionPlugin : ViewOptionPluginBase, IPluginAsViewOption
    {
        ...
        public BarCheckItem BarCheckItem
        {
            get
            {
                if (_BarCheckItem == null)
                {
                    ...
                    {
                        Caption = PluginText,
                        Glyph = Properties.Resources.LineOfWork_16x16,
                        LargeGlyph = Properties.Resources.LineOfWork_32x32,
                        Id = 601
                    };
        ...
        private void _BarItem_CheckedChanged(object sender, ItemClickEventArgs e)
        {
            ...
            if (item.Checked)
            {
                ...
                var lblResponse = Context.ServiceClient.Get(new FindApptLabels() { ViewType = "LINEOFWORK", InterfaceTypeName = typeof(IEntityWithLineOfWork).AssemblyQualifiedName });
                if (!lblResponse.IsErrorResponse())
                    SchedulerLabels = lblResponse.Labels.ConvertAll(l => l.ConvertTo<ApptLabel>());
                var stsResponse = Context.ServiceClient.Get(new FindApptStatuses() { ViewType = "LINEOFWORK", InterfaceTypeName = typeof(IEntityWithLineOfWork).AssemblyQualifiedName });
        ...
        public void AppointmentViewInfoCustomizing(object sender, AppointmentViewInfoCustomizingEventArgs e, AppointmentStatusDataStorage statuses)
        {
            try
            {
                ...
                if (e.ViewInfo.Appointment.CustomFields != null)
                    if (e.ViewInfo.Appointment.CustomFields["ENTITY"] is IEntityWithLineOfWork)
                    {
