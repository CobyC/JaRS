...
{
    [ExportPluginAsViewOption(typeof(IPluginAsViewOption), typeof(IEntityWithProgressStatus), "Status", "STATUS")]
    [PartCreationPolicy(CreationPolicy.NonShared)]
    public class StatusViewOptionPlugin : ViewOptionPluginBase, IPluginAsViewOption
    {
        ...
        public BarCheckItem BarCheckItem
        {
            get
            {
                if (_BarCheckItem == null)
                {
                    ...
                    {
                        Caption = PluginText,
                        Glyph = Properties.Resources.ViewStatus_16x16,
                        LargeGlyph = Properties.Resources.ViewStatus_32x32,
                        Id = 602
                    };
        ...
        private void _BarItem_CheckedChanged(object sender, ItemClickEventArgs e)
        {
            ...
            if (item.Checked)
            {
                ...
                var lblResponse = Context.ServiceClient.Get(new FindApptLabels() { ViewType = "STATUS", InterfaceTypeName = typeof(IEntityWithProgressStatus).AssemblyQualifiedName });
                if (!lblResponse.IsErrorResponse())
                    SchedulerLabels = lblResponse.Labels.ConvertAll(l => l.ConvertTo<ApptLabel>());
                var stsResponse = Context.ServiceClient.Get(new FindApptStatuses() { ViewType = "STATUS", InterfaceTypeName = typeof(IEntityWithProgressStatus).AssemblyQualifiedName });
