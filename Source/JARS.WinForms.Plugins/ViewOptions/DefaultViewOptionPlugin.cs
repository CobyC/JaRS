...
{
    [ExportPluginAsViewOption(typeof(IPluginAsViewOption), typeof(IEntityWithStatusLabels), "Appointment", "DEFAULT")]
    [PartCreationPolicy(CreationPolicy.NonShared)]
    public class DefaultViewOptionPlugin : ViewOptionPluginBase, IPluginAsViewOption
    {
        ...
        public BarCheckItem BarCheckItem
        {
            get
            {
                if (_BarCheckItem == null)
                {
                    ...
                    {
                        Caption = PluginText,
                        Glyph = Properties.Resources.ViewDefault_16x16,
                        LargeGlyph = Properties.Resources.ViewDefault_32x32,
                        Id = 601
                    };
        ...
        private void _BarItem_CheckedChanged(object sender, ItemClickEventArgs e)
        {
            ...
            if (item.Checked)
            {
                ...
                var lblResponse = Context.ServiceClient.Get(new FindApptLabels() { ViewType = "DEFAULT", InterfaceTypeName = typeof(IEntityWithStatusLabels).AssemblyQualifiedName });
                if (!lblResponse.IsErrorResponse())
                    SchedulerLabels = lblResponse.Labels.ConvertAll(l => l.ConvertTo<ApptLabel>());
                var stsResponse = Context.ServiceClient.Get(new FindApptStatuses() { ViewType = "DEFAULT", InterfaceTypeName = typeof(IEntityWithStatusLabels).AssemblyQualifiedName });
